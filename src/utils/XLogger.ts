...
export default class XLogger {
     ...
     * Стандартный логгер
     */
    public static default = new XLogger({mainPrefix: "Default"});
     ...
     */
    public static loggerLevel = XLogger.loggerLevels.DEBUG;
    ...
    protected _sendToConsole(message: string, prefixes: any, error: boolean = false) {
        ...
        if (this.clear) {
            if (!error) XLogger.__log(XLogger.__loggerFilter(strToDisplay, true));
            else XLogger.__error(XLogger.__loggerFilter(strToDisplay, true));
        } else if (!error) XLogger.__log(strToDisplay);
        else XLogger.__error(strToDisplay);
    }
