...
import {ColorUtils, IColorHSV, IColorRGB} from "./ColorUtils";
...
export default class Color {
     ...
     */
    public constructor(props: { hex?: string, rgb?: IColorRGB, hsv?: IColorHSV } = {}) {
        if (props.hex)
     ...
     */
    public setRGBBytes(props: { rgb: IColorRGB }) {
    ...
    public getLighterColor(percentage: number): Color {
        ...
        rgb.red = Math.round(EFMath.map(val, 0, 255, rgb.red, 255));
        rgb.green = Math.round(EFMath.map(val, 0, 255, rgb.green, 255));
        rgb.blue = Math.round(EFMath.map(val, 0, 255, rgb.blue, 255));
    ...
    public getDarkerColor(percentage: number): Color {
        ...
        rgb.red = Math.round(EFMath.map(val, 0, 255, rgb.red, 0));
        rgb.green = Math.round(EFMath.map(val, 0, 255, rgb.green, 0));
        rgb.blue = Math.round(EFMath.map(val, 0, 255, rgb.blue, 0));
        return new Color({hex: "#" + ColorUtils.rgb2hex(rgb)});
    }
